...
34. When handling images inside sections:

* Always use Shopify's `image_url` filter with a `width` parameter.
* Always use `loading="lazy"` for non-hero images.
* Use `loading="eager"` only for the first above-the-fold image.
* Always include `alt` text from settings or the image object's alt field.
* Use the `widths` parameter for responsive srcset.
